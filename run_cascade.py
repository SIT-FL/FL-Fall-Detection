...
def main(args):
    ...
    global_model = networks.CLSTM(input_size, hidden_size, num_layers, num_classes)
    ...
    save_path = f"{SAVE_FOLDER}/{'FedProx' if args.fedprox else 'FedAvg'}/{args.binary_multi}_cascade/{datetime.now().strftime('%d-%m-%Y-%H%M%S')}"
    ...
    for round in range(num_rounds):
        ...
        for i, client in enumerate(clients_train):
            ...
            local_model = networks.CLSTM(input_size, hidden_size, num_layers, num_classes).to(device)
            ...
            if args.fedprox:
                local_model = train_local_model_fedprox_cascade(global_model, local_model, client_train_loaders[i], criterion, optimizer, device, mu)
            else:
                local_model = train_local_model_cascade(local_model, client_train_loaders[i], criterion, optimizer, device)

            local_models.append(local_model)
            
            results_binary, results_multi = evaluate_cascade_model(local_model, client_test_loaders[i], criterion, device, f"round_{round + 1}_local_{client}", save_path)
        ...
        for i, unseen_client in enumerate(clients_val):
            ...
            results_binary, results_multi = evaluate_cascade_model(global_model, unseen_client_val_loaders[i], criterion, device, f"round_{round + 1}_global_unseen_{unseen_client}", save_path)
        ...
        for i, client in enumerate(clients_train):
            ...
            results_binary, results_multi = evaluate_cascade_model(global_model, client_test_loaders[i], criterion, device, f"round_{round + 1}_global_seen_{client}", save_path)
